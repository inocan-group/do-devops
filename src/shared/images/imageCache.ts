...
import { IGlobalOptions, ImageMetadata, IRefreshCacheOptions } from "~/@types";
import { IMAGE_CACHE } from "~/constants";
import { logger } from "../core";
import { currentDirectory, fileInfo } from "../file";
 ...
 * cache.
 */
let MEM_CACHE: Record<string, ImageMetadata> = {};
...
export function loadImageCache() {
  MEM_CACHE = readAndParseFile<IDictionary<ImageMetadata>>(currentDirectory(IMAGE_CACHE)) || readAndParseFile<IDictionary<ImageMetadata>>(repoDirectory(IMAGE_CACHE)) || {};
 ...
 */
export async function refreshCache(images: string[], opts: IGlobalOptions<IRefreshCacheOptions> = {}) {
  const log = logger(opts);
