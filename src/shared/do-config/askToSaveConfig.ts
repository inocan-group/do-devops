 ...
 * `do.config.js` file.
 */
export async function askToSaveConfig(configPath: string, value: any, descriptor: string = "this"): Promise<string> {
  ...
  if (answer.saveTo !== SaveChoice.doNotSave) {
    saveToConfig(configPath, value, answer.saveTo === SaveChoice.project ? "project" : "user");
  }
