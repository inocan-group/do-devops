  ...
  for (const pkg of pkgs) {
    waitFor.push(
      getExternalPackageJson(pkg)
        ...
        .catch((error) =>
          failed.push({ name: pkg, error: error instanceof Error ? error.message : String(error) })
        )
    );
  }
...
export async function otherPackages(
  ...
  for (const repo of info.successful) {
    const timing = (v: string | undefined) =>
      v ? { version: v, timing: getTagTiming(repo.value, v) } : undefined;
    ...
    const author = repo.value.author
      ? typeof repo.value.author === "string"
        ? repo.value.author
        : `${repo.value.author.name}${
            repo.value.author.email ? `<${repo.value.author.email}>` : ""
          }`
      : undefined;
    ...
    data.push({
      ...
      description: repo.value.description
        ? [repo.value.description, urlDescription].join("\n\n")
        : urlDescription,
    });
  }
  ...
  const colWidth = (col: string) => {
    ...
    const adjustedWidth = (content?: string) =>
      content ? Math.max(...colData.map((i) => i?.length)) : 0;
  ...
  const descWidth =
    available - (repoWidth + latestWidth + exportsWidth + licenseWidth + colPaddingRequirement(5));
