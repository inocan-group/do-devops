...
export async function handler(action: ValidationAction, currentBranch: string, options: IDictionary = {}) {
  ...
  if (releaseTag) console.log(chalk`- the {italic release} branch has a tag of {bold {yellow ${releaseTag}}}`);

  if (releaseTag && releaseTag !== pkgVersion) {
    console.log(chalk`- ${emoji.poop} the release branch's tag and the package.json tag DO NOT match!\n`);
    return action === "error" ? 1 : 0;
  }

  if (latest === pkgVersion) {
    console.log(chalk`- ${emoji.poop} the package.json's version must be greater than what the remote already has!\n`);
    return action === "error" ? 1 : 0;
  }
  ...
  if (localValue < remoteValue) {
    console.log(chalk`- ${emoji.poop} the package.json's version is {bold {red less than}} the remote!\n`);
    return action === "error" ? 1 : 0;
  }
