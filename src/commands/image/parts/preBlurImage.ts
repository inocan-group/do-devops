...
import { IGlobalOptions, IImageRule, SharpImageFormat, IPreBlurOptions } from "~/@types";
...
export async function preBlurImage(
  ...
  source: string,
  rule: IImageRule,
  options: IPreBlurOptions
) {
  const o = { format: "jpg" as SharpImageFormat, blur: true, size: 16, ...options };
  const log = logger(cliOptions);
  const s = getFileComponents(source);
  const sourcefile = join(rule.source, source);
  const blurFilename = join(rule.destination, s.filepath, `${s.fileWithoutExt}-blurred.jpg`);
