    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if ((!sls.hasServerlessConfig || !sls.hasProviderSection) && sls.isUsingTypescriptMicroserviceTemplate) {
            if (!sls.hasServerlessConfig) {
                console.log(chalk `- it appears that the {green serverless.yml} {italic does not} exist; will build from {italic serverless-microservice} config ${"\uD83E\uDD16" /* robot */}`);
            }
            else {
        ...
        try {
            ...
            if (!config.provider) {
                console.log(chalk `- the {red serverless.yaml} file doesn't have a {bold provider} section! ${"\uD83D\uDCA9" /* poop */}`);
            ...
            console.log(chalk `- {red serverless.yml} file is missing! ${"\uD83D\uDCA9" /* poop */}`);
