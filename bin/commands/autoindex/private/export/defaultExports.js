...
function defaultExports(exportable, opts = {}) {
    ...
    exportable.files.forEach((file) => {
        contentLines.push(`export { default as ${index_1.removeExtension(file, true)} } from "./${opts.preserveExtension ? index_1.removeExtension(file) + ".js" : index_1.removeExtension(file)}";`);
    });
    if (exportable.dirs.length > 0) {
        contentLines.push(`// directory exports`);
    }
    exportable.dirs.forEach((dir) => {
        contentLines.push(`export { default as ${index_1.removeExtension(dir, true)} } from "./${index_1.removeExtension(dir)}/index${opts.preserveExtension ? ".js" : ""}";`);
    });
